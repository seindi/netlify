...
const error_session_active_code = 4003
const error_session_active_message = "Session Exist"
...
the.account.setup = function (data, option) {
	if (data) the.account.data (data, option)
		...
		fetch.catch ()
		/*
		if (the.account.on.line ()) the.account.data.update ()
		var check = function () { if (the.account.session.check (null)) the.account.data.update () }
		the.snapshot (check, false)
...
the.account.data.update = function (data) {
	var fetch = the.fire.base.account.get ().then (function (response) {
		the.account.data (response, data)
		})
	fetch.catch ()
	}

the.account.data.format = function (data) {
	// data ["$:meta"].access = lib.json.parse (data ["$:meta"].access || "[]")
	}
...
the.account.session = function (user, password, option, context, resolve = function () {}, reject = function () {}) {
	the.fire.base.account.session.set (user, password, option).catch (reject).then (function (session) {
		...
		the.fire.base.account.get ().catch (reject).then (function (data) {
			// if (data ["$:meta"].access) {}
			// else the.account.meta.push ({access: lib.json.format ([])}).exec ()
			the.account.setup (data, {password: lib.hash.password (password)})
			resolve ({data, session})
			})
		})
	}

the.account.session.check = function (check = true) {
	if (check) {
		if (vue.session.get ("account:id")) return true
		else if (lib.storage.json ("account:session")) {
			var resolve = function () {}
			var reject = function () { the.account.logout ().exec () }
			var account = lib.storage.json ("account")
			the.account.session (account.email, account.password, {password: true}, null, resolve, reject)
			return true
			}
		else return false
		}
	else if (vue.session.get ("account:id")) return true
	else return false
	}
		...
		the.fire.base.account.email.create (user, password).catch (reject).then (function (data) {
			the.fire.store.insert ("account").set ({name: (option.name || data.name), "$:meta": {access: (option.access || [])}}, {id: data ["$:id"]}).catch (console.error).then (function () {
				// var sign_up = function () {
					// if (option.access) the.account.meta.push ({access: lib.json.format (option.access || [])})
					// }
				// if (true) the.account.login (user, password, sign_up).then (resolve).catch ()
				if (true) the.account.login (user, password).then (resolve).catch ()
				else resolve (data)
				})
			})
		})
...
the.account.login = function (user, password, context) {
	return lib.to_promise (function (resolve, reject) {
		if (the.account.on.line ()) reject ({code: error_session_active_code, message: error_session_active_message})
...
the.fire.base.account.session.on ("check", function () {
	return the.account.session.check ()
	})
...
// 64689f91652aad8cfb99
// the.fire.store.insert ("store").set ({name: "", latitude: "0", longitude: "0"}).catch ().then (console.log)
...
the.store.insert = function (name, option) {
	...
	return the.fire.store.insert ("store").set ({name, address: option.address, phone: option.phone, latitude: option.latitude, longitude: option.longitude}).catch ().then (console.log)
	}
